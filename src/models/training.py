...
class Trainer:
    ...
    def train(self, n_iters: int, loss_kwargs: dict, val_kwargs: dict = None,
        ...
        if turbo is not None:
            ...
            with tqdm(total=n_iters, desc="Training Progress",
                ...
                for loop in range(1, n_loops+1):
                    ...
                    losses_host.extend([float(x) for x in jax.device_get(losses)])
                    val_losses_host.extend([float(x) for x in jax.device_get(vals)])
                    if monitor_interval is not None:
                    ...
                    if convergence_interval is not None:
                        active_losses = val_losses_host if validation else losses_host
                        if loop * turbo > 2 * convergence_interval:
                            window1 = jnp.array(active_losses[-convergence_interval:])
                            window2 = jnp.array(active_losses[-2 * convergence_interval: -convergence_interval])
                            average1 = jnp.mean(window1)
                            average2 = jnp.mean(window2)
                            std1 = jnp.std(window1)
            ...
            with tqdm(total=n_iters, desc="Training Progress",
                ...
                for i in range(n_iters):
                    ...
                    losses_host.append(float(jax.device_get(l)))
                    val_losses_host.append(float(jax.device_get(v)) if validation else 0.0)
                    self.params_hist = params_hist
        ...
        self.losses = jnp.array(losses_host)
        self.val_losses = jnp.array(val_losses_host)
